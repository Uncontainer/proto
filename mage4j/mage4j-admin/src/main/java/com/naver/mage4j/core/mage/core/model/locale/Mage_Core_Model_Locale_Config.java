package com.naver.mage4j.core.mage.core.model.locale;
...
/**
 * MagentoNOTICE OF LICENSEThis source file is subject to the Open Software License (OSL 3.0)that is bundled with this package in the file LICENSE.txt.It is also available through the world-wide-web at this URL:http://opensource.org/licenses/osl-3.0.phpIf you did not receive a copy of the license and are unable toobtain it through the world-wide-web, please send an emailto license@magentocommerce.com so we can send you a copy immediately.DISCLAIMERDo not edit or add to this file if you wish to upgrade Magento to newerversions in the future. If you wish to customize Magento for yourneeds please refer to http://www.magentocommerce.com for more information.
 */
public class Mage_Core_Model_Locale_Config {
	/**
	 * List of allowed locales
	 */
	protected static List<String> _allowedLocales = Collections.unmodifiableList(Arrays.asList(
	 ...
	 */
	protected static List<String> _allowedCurrencies = Collections.unmodifiableList(Arrays.asList(
	...
	public static List<String> getAllowedLocales() {
		Simplexml_Element config = AppContext.getCurrent().getConfig().getNode(Mage_Core_Model_Locale.XML_PATH_ALLOW_CODES);
		if (SimplexmlUtils.isNull(config)) {
			return _allowedLocales;
		}

		Map<String, Object> configData = config.toMap();
		if (configData.isEmpty()) {
			return _allowedLocales;
		}

		List<String> result = new ArrayList<String>(_allowedLocales.size() + configData.size());
		result.addAll(_allowedLocales);
		result.addAll(configData.keySet());
	...
	public static List<String> getAllowedCurrencies() {
		Simplexml_Element config = AppContext.getCurrent().getConfig().getNode(Mage_Core_Model_Locale.XML_PATH_ALLOW_CURRENCIES);
		if (SimplexmlUtils.isNull(config)) {
		...
		List<String> result = new ArrayList<String>(_allowedCurrencies.size() + configData.size());
		result.addAll(_allowedCurrencies);
		result.addAll(configData.keySet());
